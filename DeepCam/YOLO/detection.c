...
struct obj_box* draw_detection(image im, float *box, int side, char *label)
{
    ...
    const int num = 50; // at most 7*7 = 49 predictions
    // static int bb_box[num*4]={0};
    static struct obj_box obj_bb_box[50];
    for (i=0;i<num;i++){
        obj_bb_box[i].left = YOLO_MARK;//mark
    }
    int bb_box_idx=0;
    int flag = 0;
    // --hang
    // Here the index is 20 of class score followed by 4 of bbox boundary. each bbox has a vector of 24 associated value.
    // Here the algorithm explore the fisrt side*side bboxes. j is the starting index of each bbox.
    for(r = 0; r < side; ++r){
        for(c = 0; c < side; ++c){
            j = (r*side + c) * elems;
            int class = max_index(box+j, classes); // class is the index of the type with maximum score 
            if(box[j+class] > 0.2){
                ...
                // printf("YOLO: %f %s [left,top,right,bot]:%d %d %d %d\n", box[j+class], class_names[class],left,top,right,bot);
                flag = 1;
                if (bb_box_idx<num){
                    obj_bb_box[bb_box_idx].left = left;
                    obj_bb_box[bb_box_idx].top = top;
                    obj_bb_box[bb_box_idx].right = right;
                    obj_bb_box[bb_box_idx].bot = bot;
                    obj_bb_box[bb_box_idx].score = box[j+class];
                    obj_bb_box[bb_box_idx].type = class_names[class];
                    bb_box_idx++;
                    obj_bb_box[bb_box_idx].left = YOLO_MARK; // 
                }
            }
        }
    }
